...
[Svelte](https://en.wikipedia.org/wiki/Svelte) previewed a complete rewrite of its reactivity model with Runes. [Remix](https://remix.run/) kept shipping at a pace that left most teams unsure which patterns from last year still applied. [Solid](https://www.solidjs.com/) posted benchmark numbers nobody could argue with but struggled to find adopters. [Astro](https://en.wikipedia.org/wiki/Astro_(web_framework)) carved out a strong niche for content sites. Every six months, a new meta-framework appears with different tradeoffs and a fresh set of breaking changes. Some of them are genuinely good. That is part of the problem. Good ideas still cost migration time, and migration time is not free.
